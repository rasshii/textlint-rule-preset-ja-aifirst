<!-- textlint-disable -->
<!-- 注: 本ドキュメントは preset の根拠説明という性格上、ルール名・引用文・専門用語等で textlint 違反箇所が頻出する。Phase 1.x で部分的 disable に書き換える方針 (現状はファイル全体 disable で運用)。 -->
...
主要 LLM コーディングアシスタント (Claude Code / Cursor / Devin 等） は、ベクトル検索 (RAG) ではなく ripgrep ベースの agentic search を採用している。本セクションでは、その経緯と検証された根拠を整理する。

### 1.1 Anthropic 公式の見解 (Boris Cherny, Claude Code lead)

- 出典: [Latent Space podcast (2025-05)](https://www.latent.space/p/claude-code)
- 主要引用: 「初期 Claude Code は RAG とローカルベクトル DB を試したが、agentic search が圧倒的に上回った」(意訳)
- 文脈: Claude Code は初期に RAG を試したが、コードベースに対する精度・freshness・privacy のいずれにおいても agentic search が優位だったため、ripgrep ベースの方向に舵を切った。
...
| 段落内 paraphrase 禁止 | 同義語辞書必須で実装特大 | 軽量な同義語辞書方式が確立できれば再検討 |
...
このドキュメントは preset の設計判断を追跡するため、新規ルール追加・既存ルール変更時に該当セクション (主に §4) を更新する。
...
- 新規ルール追加時: §4 (各ルールと根拠の対応関係） に行追加、§7 (Changelog) に記録
- 既存ルール変更や削除時: §4 該当行を更新、§5 (スコープ外） と一緒に再評価条件を整理、§7 に記録
