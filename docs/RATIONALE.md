...
|---|---|---|
| `prh` (内蔵 80 ルール) | 完全一致のみ hit | 表記揺れがあると同一概念に対する grep が部分的 hit にとどまる |
...
| `no-vague-heading` (独自、v1.2.0~) | 構造的関係の活用 | 見出しは AI が文書内検索の起点。短すぎる / 曖昧語の見出しは上位文脈との結合を要求し、AI grep の起点として機能不全 |
...
| `vagueWords` の default 値 | v1.2.0 では default 空配列で利用者定義に委ねる方針 | 運用シグナル (Issue / 利用報告) を見て、推奨セット (例: `['TIPS','Tips','TBD','FYI','WIP']`) を default 化するか判断 |
