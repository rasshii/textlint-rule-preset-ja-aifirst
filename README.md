...
## CI 連携 / pre-commit hook

### GitHub Actions

`.github/workflows/textlint.yml` を作成:
...
[vscode-textlint](https://marketplace.visualstudio.com/items?itemName=3w36zj6.textlint) 拡張をインストールすると、編集中にリアルタイムで警告が表示されます。
...
## `no-unbacktick-identifier` のカスタムパターン

利用側で独自パターン (関数名 / 型名 / ドメイン固有コマンド等) を追加する場合の設定例。
  ...
  "rules": {
    "preset-ja-aifirst": {
      "no-unbacktick-identifier": {
        "patterns": [
          { "source": "\\bdocker\\s+\\w+", "label": "Docker コマンド" },
          { "source": "\\bAWS::\\w+::\\w+", "label": "CloudFormation Resource" }
        ]
      }
    }
  }
...
- `source` (string, 必須): JavaScript regex の source 部分
- `flags` (string, オプション): regex flags (デフォルト `"g"`)。`g` を含まない場合は preset 側で自動付与 (無限ループ対策)
- `label` (string, オプション): エラーメッセージのプレフィックス (デフォルト「識別子」)

注意点:

- `(.+)+` のような catastrophic backtracking を起こす regex は避けてください (ReDoS リスク)
- マッチした文字列は `textlint --fix` でバッククォートで囲まれます (例: `docker run` → `` `docker run` ``)
...
独自ルールは preset 経由 (`preset-ja-aifirst/no-unbacktick-identifier`) で利用側で動作します。

textlint の rule resolver は npm package 名で rule を解決する仕様のため、リポ自身の `.textlintrc.json` には独自ルールを直接登録できません。

代わりに `tools/test-custom-rules.mjs` で inline unit test を実行します。

`@textlint/kernel` API 経由のため devDependencies 追加なしで動作 (`npm test` に組込み済み)。

検証内容:

- preset 経由で rule object が取り出せること (sanity check)
- invalid 5 ケース / valid 5 ケースで主要パターンと AST node (Code / CodeBlock / Link) をカバー

要件: Node.js 18+ (prh.YAML 内の lookbehind regex を使用）

## FAQ

### Q1. 既存の textlint preset (preset-ja-technical-writing 等) と併用できますか？

併用可能です。`.textlintrc.json` で両方を有効化し、競合するルールは利用側で個別に override してください。本 preset は AI grep 最適化に特化しているため、汎用的な技術文書ガードと組み合わせて使うことを想定しています。

### Q2. AI コーディング以外の用途 (一般的な技術ブログ等) でも使えますか？

使えます。AI grep 最適化ルール (識別子のバッククォート / 略語の括弧表記 / 弱表現排除) は人間が読む文書でも品質向上に寄与します。`no-unbacktick-identifier` は技術文書で特に有効です。

### Q3. 競合 textlint-rule-preset-ai-writing との違いは？

責務が異なります:
- **preset-ai-writing**: AI が書いた**ぽい**記述パターンを検出 (人間らしさ重視)
- **本 preset (preset-ja-aifirst)**: AI **に読ませる**ための日本語最適化 (機械可読性に特化)

両者は併用可能です。詳細は [docs/RATIONALE.md](./docs/RATIONALE.md) §5 を参照。

### Q4. `no-vague-heading` の minLength を緩めたい / 厳しくしたい

`.textlintrc.json` で override:
...
```json
{
  "rules": {
    "preset-ja-aifirst": {
      "no-vague-heading": {
        "minLength": 6,
        "vagueWords": ["TIPS", "Tips", "TBD", "FYI", "WIP"]
      }
    }
  }
}
...
`vagueWords` の default は空配列です。利用者が自プロジェクト固有の曖昧語を define する方針 (詳細は [CHANGELOG.md](./CHANGELOG.md) v1.2.0 を参照)。

### Q5. コントリビュート方法は？

[CONTRIBUTING.md](./CONTRIBUTING.md) を参照してください。Issue / PR どちらも歓迎します。
独自ルール追加時は dogfood の textlint-disable 規約とテストケース整備が必須です。
